...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    """Fast-forward failed: merge any related history so carried commits survive; reset only an
    orphaned checkout after first writing a rescue ref. ``sys.exit(1)`` on failure."""
    # Branch names do not establish ownership of commits.  A local patch can be carried directly
    # on the update target (for example main), and reset --hard would discard it when upstream
