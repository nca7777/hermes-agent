...
def restore_primary_runtime(agent) -> bool:
    ...
    if not getattr(agent, "_fallback_chain", None):
        # No chain left to fall back to: an operator/config edit removed ``fallback_providers`` out
        # from under a live activation (see GatewayRunner._apply_fallback_chain_to_agent). Both
        # gates below exist to keep the session USABLE on the fallback until the primary's reset
        # elapses — with no chain that can only keep billing a metered fallback provider the
        # operator has explicitly removed, and with nothing to alternate back to the double cache
        # invalidation they prevent cannot happen. Return to the primary now; a primary that is
        # genuinely still capped re-arms its own cooldown through the normal error path.
        agent._rate_limited_until = 0
        agent._rate_limit_backoff_count = 0
        blocked, prefetched_pool, prefetched = False, None, False
    else:
